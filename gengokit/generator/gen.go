...
import (
	...
	"github.com/gsyiven/truss/gengokit/handlers"
	templFiles "github.com/gsyiven/truss/gengokit/template"
...
func GenerateGokit(sd *svcdef.Svcdef, conf gengokit.Config) (map[string]io.Reader, error) {
	...
	// log.WithField("svcname", svcname).Debug()

	for _, templPath := range templFiles.AssetNames() {
		//log.WithField("generaet path", templPath).Debug()
		...
		//log.WithField("actualPath", actualPath).Debug()
		//log.WithField("PreviousFiles", conf.PreviousFiles[actualPath]).Debug()
